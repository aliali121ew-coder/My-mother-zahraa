...
///
/// لتحديث العملاء: ارفع نسخة جديدة من الـ APK في مجلد `releases/` مع
/// تعديل `version` في `update_config.json` ودفع التغييرات.
/// ─────────────────────────────────────────────────────────────────
...
class _UpdateDialogShellState extends State<_UpdateDialogShell> {
  ...
  Future<void> _startDownload() async {
    ...
    try {
      ...
      setState(() {
        _progress = 1;
        _statusText = 'اكتمل التنزيل — جارٍ فتح المثبّت...';
      });

      // حفظ الملف في مجلد التحميلات ثم فتحه
      ...
      setState(() {
        _downloading = false;
        _error = 'تعذر تنزيل التحديث — تحقق من الاتصال بالإنترنت';
      });
    }
  }
